...
Campanhas PAUSED/ARCHIVED não precisam estar em campaigns_analysis (mas o histórico recente delas no HISTÓRICO acima é útil pra fundamentar decisões).`;
  ...
  const response = await anthropic.messages.create({
    ...
    // 8192 (era 16384): corte de custo de API. Em contas com 8+ campanhas o
    // array de proposals pode truncar; solução de longo prazo é migrar o
    // analyzer pro Claude Code local (plano em vez de API).
    max_tokens: 8192,
    system: systemPrompt,
    tools: [{
